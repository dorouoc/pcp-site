...
// src/types.ts
var ParaType = /* @__PURE__ */ ((ParaType2) => {
  ParaType2["Project"] = "project";
  ParaType2["Resource"] = "resource";
  ParaType2["Area"] = "area";
  return ParaType2;
})(ParaType || {});
...
var CreateProjectModal = class extends import_obsidian.Modal {
  constructor(app, onSubmit) {
    super(app);
    this.INVALID_INPUT_MSG = "Name contains invalid characters: [ ] # ^ | \\ / : ?";
    this.result = {
      name: null,
      area: null
    };
  ...
  onOpen() {
    ...
    this.projectNameSetting = new import_obsidian.Setting(contentEl).setName("Name").addText((text) => text.onChange((value) => {
    ...
    this.areaNameSetting = new import_obsidian.Setting(contentEl).setName("Area").addText((text) => text.onChange((value) => {
    ...
    this.submitControl = new import_obsidian.Setting(contentEl).addButton((btn) => btn.setButtonText("Submit").setCta().onClick(() => {
      this.submit();
    }));
  }
  onClose() {
  ...
  submit() {
    if (this.result.name === null || this.result.name.length < 1) {
      new import_obsidian.Notice(error("Project name cannot be empty"));
      return;
    }
    ...
    if (!projectValid || !areaValid) {
      new import_obsidian.Notice(error("Cannot create project. Check your inputs"));
      return;
    }
    this.close();
...
async function getAllProjectFiles(app, path) {
  const items = [];
...
async function walkProjects(folder, items, app) {
  for (const child of folder.children) {
    ...
    if (child instanceof import_obsidian.TFile) {
      await app.fileManager.processFrontMatter(child, (frontMatter) => {
        ...
        if (tags !== null && tags.contains("project" /* Project */)) {
          items.push(child);
        }
      });
    }
...
async function createProject(app, settings, properties) {
  const { name, area } = properties;
  if (!name) {
    new import_obsidian.Notice(error("The project needs a name!"));
  ...
  if (!projectsPath || projectsPath && projectsPath.length === 0) {
    new import_obsidian.Notice("Projects folder path is not configured");
  ...
  const project = projects.find((project2) => project2.basename.toLowerCase() === name.toLowerCase()) || null;
  if (project && project instanceof import_obsidian.TFile) {
    await app.workspace.getLeaf().openFile(project);
    return Promise.reject("Project already exists");
  }
  const templatePath = (0, import_obsidian.normalizePath)([templatesFolder, `${projectTemplateName}.md`].join("/"));
  const projectFilePath = [projectsPath];
  if (useFolderStructure) {
    projectFilePath.push(name);
  }
  projectFilePath.push(`${name}.md`);
  const path = (0, import_obsidian.normalizePath)(projectFilePath.join("/"));
  const templateTFile = app.vault.getAbstractFileByPath(templatePath);
  if (!templateTFile) {
    new import_obsidian.Notice(error(`The template '${templatePath}' does not exist!`));
    return Promise.reject();
  }
  if (!app.vault.getAbstractFileByPath(projectsPath)) {
    app.vault.createFolder(projectsPath);
  }
  if (templateTFile instanceof import_obsidian.TFile) {
    if (useFolderStructure) {
...
var SettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Projects folder").setDesc("Where to place your projects?").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.projectsPath).setValue(this.plugin.settings.projectsPath).onChange(
        (0, import_obsidian2.debounce)(async (value) => {
          this.plugin.settings.projectsPath = (0, import_obsidian2.normalizePath)(value);
          await this.plugin.saveSettings();
        }, 500)
      )
    );
    new import_obsidian2.Setting(containerEl).setName("User folder structure for projects").setDesc("Create project folders instead of using single project file.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useFolderStructure).onChange(async (value) => {
        this.plugin.settings.useFolderStructure = value;
        await this.plugin.saveSettings();
        this.display();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Areas folder").setDesc("Where to place your areas?").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.areasPath).setValue(this.plugin.settings.areasPath).onChange(
        (0, import_obsidian2.debounce)(async (value) => {
          this.plugin.settings.areasPath = (0, import_obsidian2.normalizePath)(value);
          await this.plugin.saveSettings();
        }, 500)
      )
    );
    new import_obsidian2.Setting(containerEl).setName("Enable area companion folder").setDesc("Folder name `_<name_of_area>` which contains directly related notes to this area.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useCompanionDir).onChange(async (value) => {
        this.plugin.settings.useCompanionDir = value;
        await this.plugin.saveSettings();
        this.display();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Resources folder").setDesc("Where to place your resources?").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.resourcesPath).setValue(this.plugin.settings.resourcesPath).onChange(
        (0, import_obsidian2.debounce)(async (value) => {
          this.plugin.settings.resourcesPath = (0, import_obsidian2.normalizePath)(value);
          await this.plugin.saveSettings();
        }, 500)
      )
    );
    new import_obsidian2.Setting(containerEl).setName("Archive folder").setDesc("Where to place your archived files?").addText(
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.archivePath).setValue(this.plugin.settings.archivePath).onChange(
        (0, import_obsidian2.debounce)(async (value) => {
...
// src/init.ts
async function initializeVault(vault, settings) {
...
async function createTemplates(vault, templatesPath) {
...
var CreateAreaModal = class extends import_obsidian3.Modal {
  constructor(app, onSubmit) {
    super(app);
    this.result = {
      name: null
  ...
  onOpen() {
    ...
    const areaNameSetting = new import_obsidian3.Setting(contentEl).setName("Name").addText((text) => text.onChange((value) => {
      if (containsInvalidCharacters(value)) {
    ...
    areaNameSetting.descEl.hide();
    const submit = new import_obsidian3.Setting(contentEl).addButton((btn) => btn.setButtonText("Submit").setCta().onClick(() => {
      this.close();
      this.onSubmit(this.result);
    }));
  }
...
async function createArea(app, settings, properties) {
  if (!properties.name) {
    new import_obsidian3.Notice("The area needs a name!");
    return Promise.reject();
  }
  const folder = settings.areasPath;
  ...
  const templateTFile = app.vault.getAbstractFileByPath((0, import_obsidian3.normalizePath)(templateFile));
  if (!templateTFile) {
    new import_obsidian3.Notice(`The template '${templateFile}' does not exist!`);
    return Promise.reject();
  }
  if (templateTFile instanceof import_obsidian3.TFile) {
    let templateContent = await app.vault.cachedRead(templateTFile);
    if (!folder || !file) {
      return Promise.reject();
    }
    ...
    if (tFile && tFile instanceof import_obsidian3.TFile) {
...
var CreateResourceModal = class extends import_obsidian4.Modal {
  constructor(app, onSubmit) {
    super(app);
    this.result = {
      name: null
  ...
  onOpen() {
    ...
    const resourceNameSetting = new import_obsidian4.Setting(contentEl).setName("Name").addText((text) => text.onChange((value) => {
      if (containsInvalidCharacters(value)) {
        submit.setDisabled(true);
    ...
    resourceNameSetting.descEl.hide();
    const submit = new import_obsidian4.Setting(contentEl).addButton((btn) => btn.setButtonText("Submit").setCta().onClick(() => {
      this.close();
      this.onSubmit(this.result);
    }));
  }
...
async function createResource(app, settings, properties) {
  if (!properties.name) {
    new import_obsidian4.Notice("The resource needs a name!");
    return Promise.reject();
  }
  const folder = settings.resourcesPath;
  const file = `${folder}/${properties.name}.md`;
  const templateFile = `${settings.templatesFolder}/${settings.resourceTemplateName}.md`;
  const templateTFile = app.vault.getAbstractFileByPath((0, import_obsidian4.normalizePath)(templateFile));
  if (!templateTFile) {
    new import_obsidian4.Notice(`The template '${templateFile}' does not exist!`);
    return Promise.reject();
  }
...
// src/command-utils/archive.ts
var import_obsidian5 = require("obsidian");
async function archive(app, settings, file) {
  try {
    if (file.path.includes(settings.projectsPath)) {
      const projectTFile = file;
      await app.fileManager.processFrontMatter(projectTFile, (frontMatter) => {
        frontMatter.tags = frontMatter.tags || [];
        if (!frontMatter.tags.includes("project" /* Project */)) {
    ...
    if (file.path.includes(settings.resourcesPath)) {
      const resourceTFile = file;
      await app.fileManager.processFrontMatter(resourceTFile, (frontMatter) => {
        frontMatter.tags = frontMatter.tags || [];
        if (!frontMatter.tags.includes("resource" /* Resource */)) {
...
async function completeProject(app, settings, file) {
  const { projectsPath, archivePath } = settings;
  const isInProjectDir = file.parent !== null && file.path.contains(projectsPath);
  let hasProjectTag = false;
  await app.fileManager.processFrontMatter(file, (frontMatter) => {
    frontMatter.tags = frontMatter.tags || [];
    hasProjectTag = frontMatter.tags.contains("project" /* Project */);
  });
  if (!(isInProjectDir || hasProjectTag)) {
    new import_obsidian5.Notice("File is not an active project");
  ...
  if (file.parent.path !== projectsPath) {
    ...
    const dest = (0, import_obsidian5.normalizePath)([archivePath, file.name].join("/"));
...
async function archiveArea(app, archivePath, file) {
  const destBasePath = (0, import_obsidian5.normalizePath)([archivePath, file.basename].join("/"));
  const companionFolderPath = [
    this.app.vault.getRoot().name,
    file.path.substring(0, file.path.lastIndexOf("/")),
    `_${file.basename.toLowerCase()}`
  ].join("/");
  const companionFolder = app.vault.getAbstractFileByPath((0, import_obsidian5.normalizePath)(companionFolderPath));
  await app.vault.createFolder(destBasePath);
  await app.fileManager.renameFile(file, (0, import_obsidian5.normalizePath)([destBasePath, file.name].join("/")));
  if (companionFolder instanceof import_obsidian5.TFolder) {
    await app.fileManager.renameFile(
      companionFolder,
      (0, import_obsidian5.normalizePath)([destBasePath, companionFolder.name].join("/"))
    );
  }
}
async function archiveResource(app, archivePath, file) {
  let newFilePath = (0, import_obsidian5.normalizePath)([archivePath, file.name].join("/"));
  return app.fileManager.renameFile(file, newFilePath);
}

// src/command-utils/restore.ts
var import_obsidian6 = require("obsidian");
async function restore(app, settings, file) {
  var _a;
  try {
    if (((_a = file.parent) == null ? void 0 : _a.path) == null || !file.parent.path.contains(settings.archivePath)) {
...
async function restoreByType(app, settings, file, type) {
  try {
    switch (type) {
    ...
    new import_obsidian6.Notice(`${capitalize(type)} '${file.basename}' restored`);
  } catch (error2) {
    throw error2;
  }
}
async function restoreProject(app, settings, file) {
  ...
  if (file.parent.path !== archivePath) {
    ...
    const dest = (0, import_obsidian6.normalizePath)([projectsPath, file.name].join("/"));
    return app.fileManager.renameFile(file, dest);
  }
}
async function restoreArea(app, areasPath, file) {
  const areaArchiveBase = file.parent.path;
  const areaCompanionArchive = (0, import_obsidian6.normalizePath)([areaArchiveBase, `_${file.basename.toLowerCase()}`].join("/"));
  const areaRestorePath = (0, import_obsidian6.normalizePath)([areasPath, file.name].join("/"));
  const areaCompanionRestorePath = (0, import_obsidian6.normalizePath)([areasPath, `_${file.basename.toLowerCase()}`].join("/"));
  ...
  if (companionFolder instanceof import_obsidian6.TFolder) {
...
async function restoreResource(app, resourcesPath, file) {
  let newFilePath = (0, import_obsidian6.normalizePath)([resourcesPath, file.name].join("/"));
  return app.fileManager.renameFile(file, newFilePath);
}

// src/modals/RestoreParaItemModal.ts
var import_obsidian7 = require("obsidian");
var RestoreParaItemModal = class extends import_obsidian7.SuggestModal {
  ...
  getSuggestions(query) {
    return this.items.filter(
      (item) => item.file.basename.toLowerCase().includes(query.toLowerCase())
    );
  }
  renderSuggestion(item, el) {
    ...
    container.createEl("div", { text: (_a = item.type) == null ? void 0 : _a.toString(), cls: "para__item__type" });
  }
  onChooseSuggestion(item, _evt) {
    restoreByType(this.app, this.plugin.settings, item.file, item.type).then(() => {
    }).catch((error2) => {
      console.error("[PARA Workflower] An error occurred during restoring:", error2.message);
      new import_obsidian7.Notice(`FAILED: ${error2.message}`);
    });
  }
};

// main.ts
var ParaWorkflower = class extends import_obsidian8.Plugin {
  async onload() {
    if (!(0, import_obsidian_dataview.isPluginEnabled)(this.app)) {
      new import_obsidian8.Notice("You need to install and enable dataview first!", 5e3);
      return;
    }
    ...
    this.addCommand({
      ...
      callback: () => {
        initializeVault(this.app.vault, this.settings).then(() => {
          new import_obsidian8.Notice("Vault initialized", 5e3);
        });
      }
    });
    this.addCommand({
      ...
      callback: () => {
        new CreateProjectModal(this.app, (result) => {
          createProject(this.app, this.settings, result).then(() => {
            new import_obsidian8.Notice(`Project '${result.name}' created!`);
          }).catch((error2) => {
            if (typeof error2 == "string") {
              new import_obsidian8.Notice(error2);
            }
            console.log(error2);
          });
        }).open();
    ...
    this.addCommand({
      ...
      callback: () => {
        new CreateAreaModal(this.app, (result) => {
          createArea(this.app, this.settings, result).then(() => {
            new import_obsidian8.Notice(`Area '${result.name}' created!`);
          }).catch(() => {
          });
        }).open();
      }
    ...
    this.addCommand({
      ...
      callback: () => {
        new CreateResourceModal(this.app, (result) => {
          createResource(this.app, this.settings, result).then(() => {
            new import_obsidian8.Notice(`Resource '${result.name}' created!`);
          }).catch(() => {
          });
        }).open();
      }
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (file !== null) {
          archive(this.app, this.settings, file).then(() => {
            new import_obsidian8.Notice(`'${file.basename}' archived`);
          }).catch((error2) => {
            console.error("[PARA Workflower] An error occurred during archiving:", error2.message);
            new import_obsidian8.Notice(`FAILED: ${error2.message}`);
          });
        } else {
          new import_obsidian8.Notice("There is nothing to archive");
        }
      }
    });
    this.addCommand({
      ...
      callback: () => {
        ...
        if (file !== null && this.isArchived(file)) {
          restore(this.app, this.settings, file).then(() => {
          }).catch((error2) => {
            console.log("[PARA Workflower] An error occurred during restoring:", error2.message);
            new import_obsidian8.Notice(`FAILED: ${error2.message}`);
          });
        } else {
      ...
      callback: () => {
        ...
        if (file !== null) {
          completeProject(this.app, this.settings, file).then(() => {
            new import_obsidian8.Notice("Project completed and moved to archive");
          });
        }
      }
    });
...
async function getArchivedParaItems(app, archivePath) {
  const items = [];
  const archive2 = app.vault.getFolderByPath(archivePath);
  if (archive2) {
    await walkArchive(archive2, items, app);
  }
  return items;
}
async function walkArchive(folder, items, app) {
  for (const child of folder.children) {
    if (child instanceof import_obsidian8.TFolder && !child.name.startsWith("_")) {
    ...
    if (child instanceof import_obsidian8.TFile) {
      await app.fileManager.processFrontMatter(child, (frontMatter) => {
        const tags = frontMatter.tags || null;
        if (tags !== null) {
          const archiveItem = { file: child, type: null };
          if (tags.contains("project" /* Project */)) {
            archiveItem.type = "project" /* Project */;
          } else if (tags.contains("resource" /* Resource */)) {
            archiveItem.type = "resource" /* Resource */;
          } else if (tags.contains("area" /* Area */)) {
            archiveItem.type = "area" /* Area */;
          }
